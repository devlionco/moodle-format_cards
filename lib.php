...
// define('FORMAT_cards_COLLAPSED', 0);
// define('FORMAT_cards_EXPANDED', 1);
define('FORMAT_cards_PINNED', 1);
define('FORMAT_cards_UNPINNED', 0);
...
class format_cards extends format_base {
    ...
    public function get_view_url($section, $options = array()) {
        ...
        if (array_key_exists('sr', $options)) {
            ...
            // if ($section->collapsed == FORMAT_cards_COLLAPSED) {
    ...
    protected function find_collapsed_parent($section, $returnid = false) {
        $section = $this->get_section($section);
        if (!$section->section || $section->collapsed == FORMAT_cards_COLLAPSED) {
        ...
        static $courseformatoptions = false;
            ...
            $courseformatoptions = array(
                'displayunits' => array(
                    ...
                    'help' => get_string('displayunitsdesc', 'format_cards'),
                ...
                'displaymessages' => array(
                    ...
                    'help' => get_string('displaymessagesdesc', 'format_cards'),
                    'help_component' => 'moodle',
                ),
                'displaygrades' => array(
                    ...
                    'help' => get_string('displaygradesdesc', 'format_cards'),
                    'help_component' => 'moodle',
                ),
                'showbagestag' => array(
    ...
    public function section_format_options($foreditform = false) {
        return array(
            ...
            // 'collapsed' => array(
            //     'type' => PARAM_INT,
            //     'label' => get_string('displaycontent', 'format_cards'),
            //     'element_type' => 'select',
            //     'element_attributes' => array(
            //         array(
            //             FORMAT_cards_EXPANDED => new lang_string('showexpanded', 'format_cards'),
            //             FORMAT_cards_COLLAPSED => new lang_string('showcollapsed', 'format_cards'),
            //         )
            //     ),
            ...
            'pinned' => array(
                'type' => PARAM_INT,
                'label' => get_string('displaypinned', 'format_cards'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        FORMAT_cards_UNPINNED => new lang_string('showunpinned', 'format_cards'),
                        FORMAT_cards_PINNED => new lang_string('showpinned', 'format_cards'),
                    )
                ),
                'default' => COURSE_DISPLAY_SINGLEPAGE,
            )
     ...
     *
     * format_cards processes additional attributes in the view course URL
    ...
    public function page_set_course(moodle_page $page) {
        ...
        if ($this->on_course_view_page()) {
            ...
            //     if ($section->collapsed == FORMAT_cards_EXPANDED) {
            //         $newvalue = FORMAT_cards_COLLAPSED;
            //     } else {
            //         $newvalue = FORMAT_cards_EXPANDED;
            //     }
            //     $this->update_section_format_options(array('id' => $section->id, 'collapsed' => $newvalue));
            //     if ($newvalue == FORMAT_cards_COLLAPSED) {
            //         if (!isset($options['sr'])) {
            ...
            if ($switchpinned && confirm_sesskey() && has_capability('moodle/course:update', $context)
                ...
                if ($newvalue == FORMAT_cards_PINNED) {
                    // if (!isset($options['sr'])) {
                    //     $options['sr'] = $this->find_collapsed_parent($section->parent);
                    // }
                    redirect(course_get_url($this->courseid, $switchcollapsed, $options));
     ...
     *
     * @param int|section_info $section
     * @return array of format_cards_edit_control
     */
    public function get_section_edit_controls($section) {
        ...
        //     if ($section->collapsed == FORMAT_cards_EXPANDED) {
        //         $text = new lang_string('showcollapsed', 'format_cards');
        //         $class = 'expanded';
        //     } else {
        //         $text = new lang_string('showexpanded', 'format_cards');
        //         $class = 'collapsed';
        //     }
        //     $controls[] = new format_cards_edit_control($class, $switchcollapsedurl, $text);
        // }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            if ($section->pinned == FORMAT_cards_UNPINNED) {
                $text = new lang_string('showpinned', 'format_cards');
                $class = 'unpinned';
            } else {
                $text = new lang_string('showunpinned', 'format_cards');
                $class = 'pinned';
            }
            $controls[] = new format_cards_edit_control($class, $switchpinnedurl, $text);
        }
        ...
        if (has_capability('moodle/course:update', $context)) {
            ...
            $controls[] = new format_cards_edit_control('settings', $editurl, $text);
        }
        ...
        //     $text = new lang_string('mergeup', 'format_cards');
        //     $controls[] = new format_cards_edit_control('mergeup', $mergeupurl, $text);
        // }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            $text = new lang_string('deletesection', 'format_cards');
            $controls[] = new format_cards_edit_control('delete', $deleteurl, $text);
        }
        ...
        //     $controls[] = new format_cards_edit_control('move', $moveurl, $text);
        // }
    ...
    public function get_edit_control_movehere($parent, $before, $sr = null) {
        ...
        return new format_cards_edit_control('movehere', $movelink, $str);
    }

    /**
     * Returns a control to exit the section moving mode
     *
     * @return null|format_cards_edit_control
     */
    public function get_edit_controls_cancelmoving() {
        ...
        if ($movingsection) {
            ...
            $str = strip_tags(get_string('cancelmoving', 'format_cards', $this->get_section_name($movingsection)));
            $controls[] = new format_cards_edit_control('cancelmovingsection', $cancelmovingurl, $str);
        }
        ...
        if (ismoving($this->courseid)) {
            ...
            $str = strip_tags(get_string('cancelmoving', 'format_cards', $USER->activitycopyname));
            $controls[] = new format_cards_edit_control('cancelmovingactivity', $cancelmovingurl, $str);
        }
        return $controls;
    }
    ...
    public function get_add_section_control($parentsection) {
        ...
        if ($parentsection) {
            $text = new lang_string('addsubsection', 'format_cards');
        } else {
            $text = new lang_string('addsection', 'format_cards');
        }
        return new format_cards_edit_control('addsection', $url, $text);
    }
    ...
    public function course_content_header() {
        ...
        if ($this->get_viewed_section()) {
            ...
            if ($section->parent) {
                ...
                $text = new lang_string('backtosection', 'format_cards', $this->get_section_name($section->parent));
            } else {
                $sr = 0;
                $text = new lang_string('backtocourse', 'format_cards', $this->get_course()->fullname);
            }
            $url = $this->get_view_url($section->section, array('sr' => $sr));
            return new format_cards_edit_control('backto', $url, strip_tags($text));
        }
        ...
        if ($PAGE->context && $PAGE->context->contextlevel == CONTEXT_MODULE && $PAGE->cm) {
            ...
            if ($sectionnum) {
                $text = new lang_string('backtosection', 'format_cards', $this->get_section_name($sectionnum));
            } else {
                $text = new lang_string('backtocourse', 'format_cards', $this->get_course()->fullname);
            }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_cards_edit_control implements renderable {
    public $url;
    public $text;
    public $class;
 ...
 */
function format_cards_get_fontawesome_icon_map() {
    return [
        'format_cards:mergeup' => 'fa-level-up',
    ];
}
